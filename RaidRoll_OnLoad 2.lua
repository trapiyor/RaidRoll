...
-- Displays tooltip for rollers
	if ID ~= nil then
		...
		if RR_RollData ~= nil and RR_RollData[rr_CurrentRollID] ~= nil and RR_RollData[rr_CurrentRollID][CharID] ~= nil then
			RR_Tooltips[ID] = RollerName[rr_CurrentRollID][CharID] .. " " .. RollerRoll[rr_CurrentRollID][CharID] .. " " .. RR_RollData[rr_CurrentRollID][CharID]["LowHigh"] .. " " .. RollerDKP[rr_CurrentRollID]
		...
		if RaidRoll_DBPC[UnitName("player")]["RR_RollCheckBox_Track_Bids"] == true then
			if event == "CHAT_MSG_RAID" or event == "CHAT_MSG_RAID_LEADER" or event == "CHAT_MSG_RAID_WARNING" then
				...
				if tonumber(bid)  ~= nil then
					--if tonumber(bid) <= DKPmax then
						if tonumber(bid) ~= 123 then
...
--if rr_rollID == nil then rr_rollID = rr_CurrentRollID end 
	...
	-- Making the variable to an array
		...
		if RollerDKP[rr_rollID] == nil then RollerDKP[rr_rollID] = {} end
	...
	-- Filling in the blank values with dummies
		for i=1,MaxPlayersValue-1 do
			...
			if RollerDKP[rr_rollID][i]  == nil then RollerDKP[rr_rollID][i]  = 11 end
	...
	-- Setting the players name, roll, guild rank and roll status to the current max players value
		...
		-- Default rank and rank id values (will be overwritten if the person is in the guild
			...
			RollerDKP[rr_rollID][MaxPlayersValue] = 10
		...
		-- scan the guild info for the person 
			for i=1,numOnline do
				name, rank, rankIndex, officernote = GetGuildRosterInfo(i)
			...
			-- If a match is found set the rank and rankindex 
				if PlayerName == name then
					...
					RollerDKP[rr_rollID][MaxPlayersValue] = officernote
				end
			end		
...
temp_arr={{{}}}
	...
	temp = RollerDKP[rr_rollID][j+1]
	RollerDKP[rr_rollID][j+1] = RollerDKP[rr_rollID][j]
	RollerDKP[rr_rollID][j] = temp
